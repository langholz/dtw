...
var dpHelpers = require('./dataPointHelpers')
 ...
 * @method compute
 * @param {number[] | DataPoint[]} firstSequence The first sequence.
 * @param {number[] | DataPoint[]} secondSequence The second sequence.
...
function DTW(options) {
    ...
    function normalizeSequence(sequence, sequenceParameterName) {
        var dataPointSequence = sequence;
        if (validate.sequenceIsNumberArray(sequence)) {
            dataPointSequence = dpHelpers.dataPointArrayFromNumberArray(sequence);
        }

        validate.dataPointSequence(dataPointSequence, sequenceParameterName);
        return dataPointSequence;
    }

    this.compute = function (firstSequence, secondSequence, window) {
        validateComputeParameters(firstSequence, secondSequence);
        firstSequence = normalizeSequence(firstSequence, 'firstSequence');
        secondSequence = normalizeSequence(secondSequence, 'secondSequence');
...
function evaluateDistance(dataPoint1, dataPoint2, distanceFunction) {
    var concatenatedDataPoint = dataPoint1.concat(dataPoint2);
    var distance = concatenatedDataPoint.reduce(distanceFunction);
    return distance;
}
...
function computeOptimalPath(s, t, state) {
    ...
    distanceCostMatrix[0][0] = evaluateDistance(s[0], t[0], state.distance);

    for (var rowIndex = 1; rowIndex < state.m; rowIndex++) {
        var cost = evaluateDistance(s[rowIndex], t[0], state.distance);
        distanceCostMatrix[rowIndex][0] = cost + distanceCostMatrix[rowIndex - 1][0];
    }

    for (var columnIndex = 1; columnIndex < state.n; columnIndex++) {
        var cost = evaluateDistance(s[0], t[columnIndex], state.distance);
        distanceCostMatrix[0][columnIndex] = cost + distanceCostMatrix[0][columnIndex - 1];
    }

    for (var rowIndex = 1; rowIndex < state.m; rowIndex++) {
        for (var columnIndex = 1; columnIndex < state.n; columnIndex++) {
            var cost = evaluateDistance(s[rowIndex], t[columnIndex], state.distance);
...
function computeOptimalPathWithWindow(s, t, w, state) {
    ...
    for (var rowIndex = 1; rowIndex <= state.m; rowIndex++) {
        for (var columnIndex = Math.max(1, rowIndex - window); columnIndex <= Math.min(state.n, rowIndex + window); columnIndex++) {
            var cost = evaluateDistance(s[rowIndex - 1], t[columnIndex - 1], state.distance);
            distanceCostMatrix[rowIndex][columnIndex] =
                cost + Math.min(
                    distanceCostMatrix[rowIndex - 1][columnIndex],          // Insertion
                    distanceCostMatrix[rowIndex][columnIndex - 1],          // Deletion
